# FlipbookBatteries agent guide

FlipbookBatteries is a collection of Luau modules for use with Lute — the shared tooling helpers that the flipbook-labs `.lute/` scripts build on. It is written in Luau, runs on Lute, and is consumed by other repos as a Loom package. This file is the entry point for any agent working in the repo. It routes you to the shared skills library and the repo's own setup.
...
1. Install the toolchain and dependencies:
   ...
   `rokit install` puts `lute`, `luau-lsp`, `stylua`, and `selene` on PATH. `lute pkg install` fetches the Loom dependencies into `~/.loom/store`. The shared skills library is a dev dependency, so it lands in `~/.loom/store` for you to read but is not bundled into the package. Without this step the skills are not on disk.
...
- `lute run --list` shows the available scripts: `build` (bundle the package into `build/`), `analyze` (luau-lsp checks), and `smoketest`.
- Tests are `*.spec.luau` files alongside the code they cover. Run the suite with `lute test src/`.
